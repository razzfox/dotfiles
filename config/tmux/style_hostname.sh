...
#tmux $( grep " pane-active-border-fg " $HOME/.tmux.conf | sed "s/colour[0-9]*/colour$(color_number $HOSTNAME)/" )
#tmux $( grep " window-status-current-format " $HOME/.tmux.conf | sed "s/bg=colour[0-9]*/bg=colour$(color_number $HOSTNAME)/" )
#tmux $( grep " status-right " $HOME/.tmux.conf | sed "s/bg=colour[0-9]*/bg=colour$(color_number $HOSTNAME)/" )
